(function () {
  ...
  const state = {
    ...
    odom: null,
    robotMarker: null,
    robotMarkerTarget: null,
    mapToOdom: null,
    mapToOdomFallback: null,
    goal: null,
    keys: new Set(),
    joystick: {
      active: false,
    ...
    cameraBytesStarted: 0,
    cameraBytesReceived: 0,
    cameraLastReceivedAt: 0,
    cameraPausedByVisibility: false,
  };
  ...
  function init() {
    ...
    state.mapToOdomFallback = normalizeMapTransform(runtimeConfig.mapToOdom) || {
      x: 6.2025628587,
      y: 29.8863434457,
      yaw: 0.5406,
    };
    state.mapToOdom = { ...state.mapToOdomFallback };
    ...
    if (window.localStorage.getItem("gen0.cmdVelSchemaVersion") !== "direct-v1") {
      dom.cmdVelTopic.value = "/cmd_vel";
      window.localStorage.setItem("gen0.cmdVelSchemaVersion", "direct-v1");
    }
    ...
    dom.cameraMessageType.value =
      window.localStorage.getItem("gen0.cameraMessageType") || "compressed";
    if (dom.cameraMessageType.value === "compressed" &&
        !dom.cameraTopic.value.endsWith("/compressed")) {
      dom.cameraTopic.value = "/gen0_model/driver_camera/compressed";
    }
    dom.cameraPreset.value =
      window.localStorage.getItem("gen0.cameraPreset") || "balanced";
    if (window.localStorage.getItem("gen0.cameraSchemaVersion") !== "driver-v1") {
      dom.cameraTopic.value = "/gen0_model/driver_camera/compressed";
      dom.cameraMessageType.value = "compressed";
      window.localStorage.setItem("gen0.cameraSchemaVersion", "driver-v1");
    }
    ...
    setBadge(dom.birdviewBadge, "offline", "等待数据");
    setBadge(dom.mapBadge, "offline", "等待地图");
    setBadge(dom.navigationBadge, "offline", "空闲");
    setBadge(dom.agentBadge, "offline", "未连接");
    ...
    window.setInterval(updateRobotMarker, 333);
  ...
  function bindEvents() {
    ...
    dom.cameraPreset.addEventListener("change", () => {
      window.localStorage.setItem("gen0.cameraPreset", dom.cameraPreset.value);
      if (state.connected) {
        subscribeCamera();
      }
    });
    dom.cameraMessageType.addEventListener("change", applyCameraTypeDefaults);
    ...
    dom.goalYaw.addEventListener("input", updateGoalYawPreview);
    ...
    dom.sendAgentCommandButton.addEventListener("click", sendAgentCommand);
    dom.agentStopButton.addEventListener("click", emergencyAgentStop);
    dom.agentCommand.addEventListener("keydown", (event) => {
      if ((event.ctrlKey || event.metaKey) && event.key === "Enter") sendAgentCommand();
    });
    ...
    window.addEventListener("pagehide", () => stopRobot(false));
    window.addEventListener("beforeunload", () => stopRobot(false));
  ...
  function teardownRos() {
    ...
    state.navGoal = null;
    unsubscribe(state.agentStatus);
    if (state.agentCommand && state.agentCommand.unadvertise) state.agentCommand.unadvertise();
    if (state.controlMode && state.controlMode.unadvertise) state.controlMode.unadvertise();
    state.agentCommand = null;
    state.controlMode = null;
    state.agentStatus = null;
    state.agentStop = null;
    state.odom = null;
    state.robotMarker = null;
    state.robotMarkerTarget = null;
    state.mapToOdom = { ...state.mapToOdomFallback };
  ...
  function createAgentInterface() {
    if (!state.connected || !state.ros) return;
    state.agentCommand = new window.ROSLIB.Topic({
      ros: state.ros,
      name: "/llm_agent/command",
      messageType: "std_msgs/msg/String",
    });
    state.agentCommand.advertise();
    state.controlMode = new window.ROSLIB.Topic({
      ros: state.ros,
      name: "/epsilon/control_mode",
      messageType: "std_msgs/msg/String",
    });
    state.controlMode.advertise();
    state.agentStatus = new window.ROSLIB.Topic({
      ros: state.ros,
      name: "/llm_agent/status",
      messageType: "std_msgs/msg/String",
    });
    state.agentStatus.subscribe((message) => {
      try {
        const status = JSON.parse(message.data);
        const parsedTasks = status.plan?.tasks;
        const planDetail = Array.isArray(parsedTasks)
          ? parsedTasks.map((task) => {
              const parameters = Object.entries(task.parameters || {})
                .map(([key, value]) => `${key}=${value}`)
                .join(", ");
              return `${task.skill}(${parameters})`;
            }).join(" → ")
          : "";
        const detail = status.error || (planDetail ? `解析结果：${planDetail}` : status.event || "已更新");
        dom.agentStatus.textContent = detail;
        if (status.plan) {
          dom.agentPlan.textContent = JSON.stringify(status.plan, null, 2);
        } else if (status.error) {
          dom.agentPlan.textContent = JSON.stringify({ error: status.error }, null, 2);
        }
        setBadge(dom.agentBadge, status.event === "failure" || status.event === "rejected" ? "error" : "online", status.event || "在线");
        appendLog(`Agent: ${detail}`, status.error ? "error" : "info");
      } catch (error) {
        dom.agentStatus.textContent = message.data;
      }
    });
    state.agentStop = new window.ROSLIB.Service({
      ros: state.ros,
      name: "/llm_agent/stop",
      serviceType: "std_srvs/srv/Trigger",
    });
    setBadge(dom.agentBadge, "online", "已连接");
    dom.agentStatus.textContent = "可提交自然语言任务";
  }

  function sendAgentCommand() {
    const command = dom.agentCommand.value.trim();
    if (!state.connected || !state.agentCommand) {
      appendLog("请先连接 rosbridge，再提交智能任务。", "warn");
      return;
    }
    if (!command) {
      appendLog("任务内容不能为空。", "warn");
      return;
    }
    state.agentCommand.publish(new window.ROSLIB.Message({ data: command }));
    dom.agentStatus.textContent = "任务已提交，等待 Agent 校验";
    dom.agentPlan.textContent = "等待模型解析...";
    setBadge(dom.agentBadge, "connecting", "规划中");
    appendLog(`提交智能任务：${command}`);
  }

  function emergencyAgentStop() {
    if (!state.connected || !state.agentStop) {
      stopRobot(true);
      appendLog("Agent stop 服务不可用，已执行本地零速度停止。", "warn");
      return;
    }
    state.agentStop.callService(new window.ROSLIB.ServiceRequest({}), (response) => {
      stopRobot(false);
      dom.agentStatus.textContent = response.message || "车辆已停止";
      setBadge(dom.agentBadge, response.success ? "active" : "error", response.success ? "已停止" : "停止失败");
      appendLog(response.message || "Agent 紧急停止完成。", response.success ? "warn" : "error");
    }, (error) => {
      stopRobot(true);
      appendLog(`Agent stop 调用失败，已执行本地停止：${error}`, "error");
    });
  }
  ...
  function setupSpeechRecognition() {
    ...
    if (!SpeechRecognition) {
      ...
      dom.voiceButton.textContent = "浏览器不支持语音";
      return;
    }
    ...
    recognition.onstart = () => {
      ...
      dom.voiceButton.textContent = "■ 停止录音";
      dom.voiceButton.setAttribute("aria-pressed", "true");
      setBadge(dom.agentBadge, "active", "聆听中");
    };
    recognition.onresult = (event) => {
      ...
      dom.agentCommand.value = transcript.trim();
    };
    recognition.onerror = (event) => appendLog(`语音识别失败：${event.error}`, "error");
    recognition.onend = () => {
      state.recognizing = false;
      dom.voiceButton.textContent = "🎙 开始语音";
      dom.voiceButton.setAttribute("aria-pressed", "false");
      setBadge(dom.agentBadge, state.connected ? "online" : "offline", state.connected ? "待确认" : "未连接");
    };
  ...
  function subscribeCamera() {
    ...
    unsubscribe(state.camera);
    state.camera = null;
    state.cameraLatestFrame = null;
    const topicName = dom.cameraTopic.value.trim() || "/gen0_model/driver_camera";
    const compressed = dom.cameraMessageType.value === "compressed";
    if (compressed && !topicName.endsWith("/compressed")) {
      setBadge(dom.cameraBadge, "error", "话题类型不匹配");
      appendLog("CompressedImage 必须使用 /compressed 话题，已阻止错误订阅。", "error");
      return;
    }
    if (!compressed && topicName.endsWith("/compressed")) {
      setBadge(dom.cameraBadge, "error", "话题类型不匹配");
      appendLog("RAW Image 不能订阅 /compressed 话题，已阻止错误订阅。", "error");
      return;
    }
    const throttleRates = { eco: 500, balanced: 250, smooth: 125 };
    const throttleRate = throttleRates[dom.cameraPreset.value] || 250;
    window.localStorage.setItem("gen0.cameraTopic", topicName);
    window.localStorage.setItem("gen0.cameraMessageType", compressed ? "compressed" : "image");
    ...
    appendLog(
      `相机已订阅 ${topicName} (${compressed ? "JPEG" : "RAW"}, ${Math.round(1000 / throttleRate)} FPS 上限)。`,
    );
  }

  function subscribeBirdview() {
    unsubscribe(state.birdview);
    state.birdview = new window.ROSLIB.Topic({
      ros: state.ros,
      name: "/scene_overview/compressed",
      messageType: "sensor_msgs/msg/CompressedImage",
      throttle_rate: 500,
      queue_length: 1,
    });
    setBadge(dom.birdviewBadge, "connecting", "订阅中");
    state.birdview.subscribe((message) => {
      if (!message || !message.data) return;
      dom.birdviewImage.src = `data:image/jpeg;base64,${message.data}`;
      dom.birdviewPlaceholder.hidden = true;
      setBadge(dom.birdviewBadge, "online", "实时画面");
    });
  }

  function handleCameraMessage(message) {
    const receivedAt = performance.now();
    state.cameraLastReceivedAt = receivedAt;
    state.cameraBytesReceived += estimateMessageBytes(message);
    if (!state.cameraBytesStarted) {
      state.cameraBytesStarted = receivedAt;
    }
    if (state.cameraLatestFrame || state.cameraRendering) {
      state.cameraDroppedFrames += 1;
      dom.cameraDroppedLabel.textContent = String(state.cameraDroppedFrames);
    }
    state.cameraLatestFrame = { message, receivedAt };
    scheduleCameraRender();
  }

  function scheduleCameraRender() {
    if (state.cameraRenderScheduled || state.cameraRendering || document.hidden) {
      return;
    }
    state.cameraRenderScheduled = true;
    window.requestAnimationFrame(renderLatestCameraFrame);
  }

  async function renderLatestCameraFrame() {
    state.cameraRenderScheduled = false;
    const frame = state.cameraLatestFrame;
    if (!frame || document.hidden) {
      return;
    }
    state.cameraLatestFrame = null;
    state.cameraRendering = true;
    try {
      if (dom.cameraMessageType.value === "compressed") {
        await renderCompressedImage(frame.message);
      } else {
        renderRawImage(frame.message);
      }
      updateCameraRate();
      const latency = Math.max(0, performance.now() - frame.receivedAt);
      dom.cameraLatencyLabel.textContent = `${latency.toFixed(0)} ms`;
      dom.cameraStreamLabel.textContent = "LIVE";
      setBadge(dom.cameraBadge, "online", "图像正常");
      dom.lastMessage.textContent = "作业视觉";
    } catch (error) {
      setBadge(dom.cameraBadge, "error", "图像解析失败");
      dom.cameraStreamLabel.textContent = "ERROR";
      appendLog(`相机帧解析失败: ${error.message}`, "error");
    } finally {
      state.cameraRendering = false;
      if (state.cameraLatestFrame) {
        scheduleCameraRender();
      }
    }
  }
  ...
  async function renderCompressedImage(message) {
    const bytes = toUint8Array(message.data);
    ...
    if (typeof window.createImageBitmap === "function") {
      ...
      dom.cameraResolutionLabel.textContent = `${bitmap.width} x ${bitmap.height}`;
      dom.cameraPlaceholder.hidden = true;
      bitmap.close();
      return;
    }
    await renderCompressedImageFallback(blob, format);
  }

  function renderCompressedImageFallback(blob, format) {
    return new Promise((resolve, reject) => {
      ...
      image.onload = () => {
        ...
        dom.cameraEncodingLabel.textContent = format;
        dom.cameraResolutionLabel.textContent = `${image.naturalWidth} x ${image.naturalHeight}`;
        dom.cameraPlaceholder.hidden = true;
      ...
      image.onerror = () => {
        URL.revokeObjectURL(url);
        reject(new Error(`无法解码 ${format} 图像`));
      };
      image.src = url;
    });
  }
  ...
  function clearCamera() {
    ...
    state.cameraFpsFrames = 0;
    state.cameraBytesStarted = 0;
    state.cameraBytesReceived = 0;
    state.cameraLastReceivedAt = 0;
    state.cameraDroppedFrames = 0;
    dom.cameraFpsLabel.textContent = "-- FPS";
    dom.cameraStreamLabel.textContent = "STANDBY";
    dom.cameraLatencyLabel.textContent = "-- ms";
    dom.cameraDroppedLabel.textContent = "0";
    dom.cameraBandwidthLabel.textContent = "-- Mbps";
  ...
  function applyCameraTypeDefaults() {
    const compressed = dom.cameraMessageType.value === "compressed";
    dom.cameraTopic.value = compressed
      ? "/gen0_model/driver_camera/compressed"
      : "/gen0_model/driver_camera";
    window.localStorage.setItem("gen0.cameraMessageType", dom.cameraMessageType.value);
    if (state.connected) {
      subscribeCamera();
    }
  }
  ...
  function handleVisibilityChange() {
    if (document.hidden) {
      // Keep the rosbridge subscription alive while the tab is in the
      // background.  Repeated unsubscribe/subscribe cycles were unreliable
      // on remote links and could leave the camera with zero subscribers.
      // Rendering is already suspended by scheduleCameraRender().
      state.cameraPausedByVisibility = Boolean(state.camera);
      if (state.cameraPausedByVisibility) dom.cameraStreamLabel.textContent = "PAUSED";
      stopRobot(false);
    } else if (state.connected && state.cameraPausedByVisibility) {
      state.cameraPausedByVisibility = false;
      scheduleCameraRender();
    }
  }
  ...
  function updateCameraHealth() {
    const now = performance.now();
    if (state.cameraBytesStarted && now - state.cameraBytesStarted >= 1000) {
      const mbps = (state.cameraBytesReceived * 8) / (now - state.cameraBytesStarted) / 1000;
      dom.cameraBandwidthLabel.textContent = `${mbps.toFixed(2)} Mbps`;
      state.cameraBytesStarted = now;
      state.cameraBytesReceived = 0;
    }
    if (state.connected && state.camera && state.cameraLastReceivedAt &&
        now - state.cameraLastReceivedAt > 1500) {
      setBadge(dom.cameraBadge, "error", "视频中断");
      dom.cameraStreamLabel.textContent = "STALE";
    }
  }
  ...
  function estimateMessageBytes(message) {
    ...
    if (typeof message.data === "string") {
      return Math.floor(message.data.length * 0.75);
    }
  ...
  async function loadStaticMap() {
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        const imageUrl = new URL(imageName, staticMapUrl);
        imageUrl.searchParams.set("map", "pcd-full-v2");
  ...
  function handleMapMessage(message) {
    ...
    // The preview image is only available before ROS connects.  Once Nav2
    // publishes /map it must become the canvas source: its origin and extent
    // are the same coordinates used by /odom and /navigate_to_pose.
    state.map = rosMap;
    state.staticMapLoaded = false;
    dom.mapInfo.textContent =
  ...
  function handleOdomMessage(message) {
    ...
    state.robotMarkerTarget = state.odom;
  ...
  function updateRobotPoseReadout() {
    const displayPose = state.robotMarker || state.odom;
    if (!displayPose) {
      return;
    }
    dom.robotPose.textContent =
      `${displayPose.x.toFixed(2)}, ${displayPose.y.toFixed(2)}`;
    dom.robotYaw.textContent = `${radToDeg(displayPose.yaw).toFixed(1)} deg`;
  }

  function updateRobotMarker() {
    const target = state.robotMarkerTarget;
    if (!target) {
      return;
    }
    if (!state.robotMarker) {
      state.robotMarker = { ...target };
    } else {
      const blend = 0.45;
      state.robotMarker.x += (target.x - state.robotMarker.x) * blend;
      state.robotMarker.y += (target.y - state.robotMarker.y) * blend;
      state.robotMarker.yaw = normalizeAngle(
        state.robotMarker.yaw
        + normalizeAngle(target.yaw - state.robotMarker.yaw) * blend,
      );
      if (Math.hypot(
        target.x - state.robotMarker.x,
        target.y - state.robotMarker.y,
      ) < 0.01) {
        state.robotMarker = { ...target };
      }
    }
    updateRobotPoseReadout();
    drawMap();
  }
  ...
  function drawMap() {
    ...
    const robotMarker = state.robotMarker || state.odom;
    if (robotMarker) {
      const point = worldToCanvas(robotMarker.x, robotMarker.y);
      drawPoseMarker(context, point.x, point.y, robotMarker.yaw, "#ef9f35", "机器人");
    }
    if (state.goal) {
  ...
  function canvasToWorld(event) {
    ...
    const mapY = state.map.height - 1 - (y - offsetY) / scale;
    if (
      mapX < 0 ||
      mapY < 0 ||
  ...
  function worldToCanvas(x, y) {
    ...
    return {
      ...
      y: offsetY + (state.map.height - 1 - mapY) * scale,
    };
  }
  ...
  function updateGoalYawPreview() {
    const yawDegrees = Number(dom.goalYaw.value);
    if (!state.goal || !Number.isFinite(yawDegrees)) {
      return;
    }
    state.goal.yaw = degToRad(yawDegrees);
    drawMap();
  }
  ...
  function publishTeleopCommand() {
    ...
    if (moving || state.wasMoving) {
      if (moving && state.controlMode) {
        state.controlMode.publish(new window.ROSLIB.Message({ data: "teleop" }));
      }
  ...
  function stopRobot(cancelGoal) {
    ...
    if (state.controlMode) {
      state.controlMode.publish(new window.ROSLIB.Message({ data: "stop" }));
    }
  ...
  function normalizeMapTransform(value) {
