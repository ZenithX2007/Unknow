...
def project_to_2d(points):
    ...
    gx = np.floor((pts[:, 0] - min_x) / RESOLUTION).astype(np.int64)
    gy = np.floor((pts[:, 1] - min_y) / RESOLUTION).astype(np.int64)
    valid = (gx >= 0) & (gx < width) & (gy >= 0) & (gy < height)
    gx, gy, z = gx[valid], gy[valid], pts[valid, 2]

    # PCD projection has three states: unseen (gray), traversable ground
    # (near-white), and obstacles above the ground band (dark).
    ground = np.zeros((height, width), dtype=np.uint8)
    obstacle = np.zeros((height, width), dtype=np.uint8)
    ground[gy[z <= GROUND_MAX_Z], gx[z <= GROUND_MAX_Z]] = 1
    obstacle[gy[z >= OBSTACLE_MIN_Z], gx[z >= OBSTACLE_MIN_Z]] = 1

    # Close only single-cell sampling gaps; obstacles always take precedence.
    ground = np.asarray(
        Image.fromarray(ground * 255).filter(ImageFilter.MaxFilter(3))
    ) > 0
    obstacle = np.asarray(
        Image.fromarray(obstacle * 255).filter(ImageFilter.MaxFilter(3))
    ) > 0
    occupancy = np.full((height, width), 128, dtype=np.uint8)
    occupancy[ground] = 245
    occupancy[obstacle] = 28
    return np.flipud(occupancy), min_x, min_y
