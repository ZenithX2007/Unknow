...
ACTOR_SOFT_STOP_MARGIN="${GEN0_ACTOR_SOFT_STOP_MARGIN:-1.5}"
ACTOR_SOFT_STOP_RELEASE_MARGIN="${GEN0_ACTOR_SOFT_STOP_RELEASE_MARGIN:-1.9}"
ACTOR_SOFT_STOP_VEHICLE_NAME="${GEN0_ACTOR_SOFT_STOP_VEHICLE_NAME:-gen0_model}"
TRASH_SCENARIO_PATH="${GEN0_TRASH_SCENARIO_PATH:-}"

# An explicitly empty value is intentional in full-stack mode: actors are
# provided through the lightweight actor costmap, not the registered lidar.
if [[ -z "${GEN0_DYNAMIC_ACTOR_TOPICS+x}" && -z "$DYNAMIC_ACTOR_TOPICS" && -n "$ACTORS_SCENARIO" ]]; then
...
start_process relocalization_3d_slam \
  env \
    ...
    GEN0_ACTOR_COSTMAP_POSE_TOPICS="$ACTOR_COSTMAP_POSE_TOPICS" \
