 ...
 * @author GW
 * @brief 单车道前向仿真器接口：基于 IDM 与 Pure Pursuit 的车辆状态推进工具
 * @version 0.1
 * @date 2019
 * @copyright Copyright (c) 2019
 *
 * **文件概述**：
 * 本文件定义 `OnLaneForwardSimulation`，
 * 用于在给定参考车道和驾驶参数下，对车辆进行单步或多步状态推进。
 *
 * **核心用途**：
 * - MPDM/EUDM 中的闭环前向仿真
 * - 开环轨迹预测
...
class OnLaneForwardSimulation {
 public:
  ...
  static ErrorType GetTargetStateOnTargetLane(
    ...
    // Build a virtual state inside the target-lane gap.  This carries the
    // discrete lane-change decision into the continuous longitudinal rollout.
  ...
  static ErrorType PropagateOnceAdvancedLC(
    ...
    // * Step II: Calculate velocity
    // During a lane change, Context IDM tracks a target state in the chosen
    // gap while the current-lane leader still constrains collision avoidance.
 ...
 private:
  ...
  static ErrorType CalculateDesiredState(const State& current_state,
                                         ...
                                         const decimal_t dt, const Param& param,
                                         State* state) {
    // This is the common final stage of every rollout: semantic behavior has
    // already selected a reference/velocity, and the bounded bicycle model
    // turns it into one physically feasible next state.
