 ...
 * @file semantics.h
 * @author GW
 * @brief 通用语义数据结构头文件：车辆、车道语义、行为、障碍物与交通信息定义
 * @version 0.1
 * @date 2019-03-17
 ...
 * **文件概述**：
 * 本文件定义项目中的核心语义数据结构，
 * 包括车辆参数、车辆对象、语义车道、语义车辆集合、行为标签和交通约束信息。
 */
...
class VehicleParam {
 public:
  // d_cr 是几何中心到后轴中心的纵向距离，车辆状态位置默认在后轴中心。
...
class Vehicle {
 public:
  ...
  /**
   * @brief 返回后轴中心处的三自由度状态 [x, y, yaw]
   *
   * 这是车辆模型、车道匹配和 Frenet 投影默认使用的参考点。
   */
  Vec3f Ret3DofState() const;

  /**
   * @brief 返回车辆在世界坐标系中的二维有向包围盒
   *
   * 包围盒中心会根据 d_cr 从后轴中心前移，适用于碰撞检查。
   */
  OrientedBoundingBox2D RetOrientedBoundingBox() const;

  // 返回车辆四个轮廓顶点，供栅格化和可视化使用。
  ErrorType RetVehicleVertices(vec_E<Vec2f> *vertices) const;

  // 返回车辆纵向轴线上的前后保险杠点，供 IDM 净间距计算使用。
  ErrorType RetBumperVertices(std::array<Vec2f, 2> *vertices) const;

  // 返回几何中心处的三自由度状态，区别于默认的后轴中心状态。
...
/**
 * @brief 描述自车高层意图及其下游参考数据
 *
 * `lat_behavior/lon_behavior` 是离散语义，`ref_lane` 是与该行为匹配的
 * 连续参考线，`forward_trajs` 则是行为层提供给 SSC 的离散前向状态。
 * 因此该结构是行为规划和运动规划之间的主要交接对象。
 */
