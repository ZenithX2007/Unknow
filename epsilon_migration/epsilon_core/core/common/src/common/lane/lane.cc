 ...
 * @file lane.cc
 * @author GW
 * @brief 车道几何对象实现：样条车道的弧长、曲率与几何导数查询
 * @version 0.1
 * @date 2019
 * @copyright Copyright (c) 2019
 *
 * **文件概述**：
 * 本文件实现 `common::Lane` 的几何查询逻辑，
 * 支持按弧长查询位置、切向、法向、朝向和曲率等核心几何量。
 */
...
ErrorType Lane::GetArcLengthByVecPosition(const Vecf<LaneDim>& vec_position,
  ...
  // 第一阶段：在样条域中粗搜索，给 Newton 迭代选取较近的局部极小值。
...
ErrorType Lane::GetArcLengthByVecPositionWithInitialGuess(
  ...
  // 对距离平方的一阶导数求根；初值已被上层粗搜索限制在正确局部区域。
