 ...
 * @author GW
 * @brief MPDM/规则型行为规划器实现：候选行为生成、多策略前向仿真与 winner 选择
 * @version 0.1
 * @date 2019
 * @copyright Copyright (c) 2019
 *
 * **文件概述**：
 * 本文件实现 `BehaviorPlanner` 的核心逻辑，
 * 主要面向 lane keeping / lane change 等离散高层行为决策。
 *
 * **核心功能**：
 * 1. 读取 ego、车道和导航上下文
 * 2. 构造可用横向行为集合
 * 3. 使用 MPDM/多策略前向仿真评估候选行为
 * 4. 选择最优行为并输出参考车道与前向轨迹
 *
 * **系统定位**：
 * 这是 EUDM 之前的一条高层决策链实现，
 * 仍然通过前向仿真和代价评估来选择行为，但动作空间更直接、工程逻辑更轻量。
 */
...
// 运行 route planner，得到当前导航路径上的参考车道上下文。
ErrorType BehaviorPlanner::RunRoutePlanner(const int nearest_lane_id) {
  TicToc timer_rp;
  // ~ Run route planner
  std::vector<int> fixed_navi_path;
  if (map_itf_->GetFixedNavigationPath(&fixed_navi_path) == kSuccess &&
      !fixed_navi_path.empty()) {
    p_route_planner_->set_navi_mode(RoutePlanner::NaviMode::kAssignedTarget);
    p_route_planner_->set_assigned_navi_path(fixed_navi_path);
  } else {
    p_route_planner_->set_navi_mode(RoutePlanner::NaviMode::kRandomExpansion);
    p_route_planner_->set_assigned_navi_path(std::vector<int>());
  }
