 ...
 * @author GW
 * @brief 行为规划 ROS 服务端实现：在线 MPDM 节点封装、输入缓冲与回调分发
 * @version 0.1
 * @date 2019
 * @copyright Copyright (c) 2019
 *
 * **文件概述**：
 * 本文件实现 `BehaviorPlannerServer`，
 * 负责把 `BehaviorPlanner` 组织成后台运行的在线行为规划节点。
 *
 * **核心功能**：
 * 1. 缓存语义地图输入并只消费最新帧
 * 2. 以固定频率驱动行为规划循环
 * 3. 处理 joystick/HMI 干预输入
 * 4. 将规划结果通过回调转发给下游模块
 * 5. 发布候选前向轨迹可视化
 */
...
// 初始化 behavior planner、joystick 订阅器和可视化器。
