 ...
 * @file map_adapter.h
 * @author GW
 * @brief BehaviorPlanner 地图适配器接口：将 SemanticMapManager 转为 MPDM 可用地图视图
 * @version 0.1
 * @date 2019
 * @copyright Copyright (c) 2019
 *
 * **文件概述**：
 * 本文件声明 `BehaviorPlannerMapAdapter`，
 * 用于把 `SemanticMapManager` 中的语义地图、车道网络和车辆集合，
 * 适配成 `BehaviorPlannerMapItf` 所定义的统一访问接口。
 */
...
/**
 * @brief BehaviorPlanner 地图适配器
 *
 * 在系统分层上，它负责承接 `SemanticMapManager`，
 * 并为行为规划层暴露 ego 状态、车道拓扑、碰撞检测、关键车提取等能力。
 */
class BehaviorPlannerMapAdapter : public BehaviorPlannerMapItf {
 public:
  ...
  ErrorType GetFixedNavigationPath(std::vector<int> *navi_path) override;
