...
void IdealSteerModel::TruncateControl(const decimal_t &dt) {
  // Convert the desired speed/steering command into accelerations and rates
  // that a vehicle can realize in one rollout step.  EUDM relies on this
  // bounded propagation so policy comparisons remain dynamically plausible.
  // truncate velocity
  ...
  // Limit lateral acceleration and jerk before converting them back to a
  // steering angle.  Applying this in acceleration space avoids unrealizable
  // high-curvature commands at the current speed.
  desired_lat_acc_ =
...
void IdealSteerModel::Step(double dt) {
  // Keep curvature and steering synchronized before integrating the bicycle
  // model; state_ may originate from a map or controller that stores either.
  ...
  // Integrate only after all longitudinal/lateral limits have been enforced.
