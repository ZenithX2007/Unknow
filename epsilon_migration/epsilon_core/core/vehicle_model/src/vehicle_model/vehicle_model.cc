...
void VehicleModel::Step(double dt) {
  // Integrate the kinematic bicycle state [x, y, heading, steer, velocity].
  // This is the unconstrained low-level plant; the planner normally uses
  // IdealSteerModel below when it needs control-rate and comfort limits.
...
void VehicleModel::operator()(const InternalState &x, InternalState &dxdt,
  ...
  // Standard rear-axle bicycle model.  Steering-rate and longitudinal
  // acceleration are controls, so they form the last two state derivatives.
