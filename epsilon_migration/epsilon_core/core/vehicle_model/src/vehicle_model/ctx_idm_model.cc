...
void ContextIntelligentDriverModel::Step(double dt) {
  // Context IDM augments ordinary car following with a virtual target state,
  // which is used by lane-change rollouts to pull the ego vehicle into a gap.
...
void ContextIntelligentDriverModel::operator()(const InternalState &x,
  ...
  // The target tracker is intentionally bounded: it expresses the desire to
  // reach the selected gap without overpowering the surrounding traffic model.
  decimal_t v_ref =
  ...
  // The current implementation uses the context-tracking term as the active
  // command. `acc_idm` is retained above as the ordinary IDM reference for
  // future combinations of car-following and gap-tracking policies.
