...
void IntelligentDriverModel::Step(double dt) {
  // The lead vehicle state is propagated together with the ego state so IDM
  // can evaluate the gap continuously during numerical integration.
...
void IntelligentDriverModel::operator()(const InternalState &x,
  ...
  common::IntelligentDriverModel::GetAccDesiredAcceleration(param_, cur_state,
                                                            &acc);
  // Do not let a single integration step command a speed below zero.  This
  // makes the simplified longitudinal model suitable for rollout evaluation.
  acc = std::max(acc,
                 -std::min(param_.kHardBrakingDeceleration, cur_state.v / dt));
  dxdt[0] = cur_state.v;
  dxdt[1] = acc;
  dxdt[2] = cur_state.v_front;
  // In this standalone IDM model the leader is an exogenous constant-speed
  // obstacle; EUDM's closed-loop simulator updates each agent separately.
  dxdt[3] = 0.0;
}
