 ...
 * @author GW
 * @brief 语义地图管理器实现：环境融合、语义标注与基础预测
 * @version 0.1
 * @date 2019
 * @copyright Copyright (c) 2019
 *
 * **文件概述**：
 * 本文件实现 `SemanticMapManager` 的核心逻辑，
 * 包括环境快照更新、语义车道构建、关键车筛选、行为预测和开环轨迹预测。
 *
 * **系统定位**：
 * 它是整个规划系统的上游环境中台，
 * 为 MPDM、EUDM、SSC 以及执行节点提供统一的语义地图输入。
 */
...
ErrorType SemanticMapManager::UpdateSemanticMap(
  ...
  // Build one internally consistent snapshot before planners query it.  This
  // prevents behavior and motion planning from mixing lane/vehicle data from
...
ErrorType SemanticMapManager::UpdateSemanticVehicles() {
  // Attach lane-relative state and a lightweight behavior hypothesis to each
  // surrounding vehicle.  EUDM uses this semantic form when realizing and
  // branching interactive forward-simulation scenarios.
...
ErrorType SemanticMapManager::OpenloopTrajectoryPrediction() {
  // These predictions are deliberately open-loop.  They provide a conservative
  // baseline for collision queries; EUDM later replaces them with closed-loop
  // interaction rollouts for its candidate policies.
...
ErrorType SemanticMapManager::GetNearestLaneIdUsingState(
  ...
  const auto effective_navi_path = ResolveNavigationPath(navi_path);
  ...
  std::set<std::tuple<decimal_t, decimal_t, decimal_t, int>>
      lanes_in_effective_path;
  if (!effective_navi_path.empty()) {
    const std::unordered_set<int> path_lane_ids(effective_navi_path.begin(),
                                                effective_navi_path.end());
    for (const auto &ele : lanes_in_dist) {
      if (path_lane_ids.count(std::get<3>(ele)) > 0) {
        lanes_in_effective_path.insert(ele);
      }
    }
  }
  const auto &candidate_lanes =
      lanes_in_effective_path.empty() ? lanes_in_dist : lanes_in_effective_path;
  ...
  std::set<std::tuple<decimal_t, decimal_t, decimal_t, int>>
      lanes_in_angle_diff;
  for (const auto &ele : candidate_lanes) {
  ...
  if (lanes_in_angle_diff.empty() ||
    ...
    *id = std::get<3>(*candidate_lanes.begin());
    *distance = std::get<0>(*candidate_lanes.begin());
    *arc_len = std::get<1>(*candidate_lanes.begin());
    // printf(
  ...
  *arc_len = std::get<2>(*lanes_in_angle_diff.begin());
  ...
  if (std::get<3>(*lanes_in_angle_diff.begin()) !=
      std::get<3>(*candidate_lanes.begin())) {
    // printf(
...
ErrorType SemanticMapManager::UpdateKeyVehicles() {
  // Limit expensive interaction reasoning to vehicles on lanes that are
  // reachable within the planning horizon.  This is the map-side counterpart
  // of the guided branching idea: irrelevant traffic is excluded early.
      ...
      while (accum_dist_backward < max_backward_dist) {
        ...
        if (!father_ids.empty()) {
          ...
          for (auto &id : father_ids) {
            if (std::find(effective_navi_path.begin(), effective_navi_path.end(),
                          id) != effective_navi_path.end()) {
              father_id = id;
              break;
            }
          }
...
std::vector<int> SemanticMapManager::ResolveNavigationPath(
    const std::vector<int> &navi_path) const {
  if (!navi_path.empty()) {
    return navi_path;
  }
  if (agent_config_info_.enable_fixed_navi_path) {
    return agent_config_info_.fixed_navi_path;
  }
  return {};
}
