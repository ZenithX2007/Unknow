 ...
 * @author GW
 * @brief 语义地图管理器接口：融合车道、障碍物、车辆与预测信息的统一环境快照
 * @version 0.1
 * @date 2019
 * @copyright Copyright (c) 2019
 *
 * **文件概述**：
 * 本文件声明 `SemanticMapManager`，
 * 它是整个 EPSILON 规划栈的环境中台，负责把底层仿真/感知输入
 * 组织成带有拓扑、语义与预测信息的统一地图快照。
 *
 * **核心功能**：
 * 1. 维护 ego、周围车、障碍物和车道网
 * 2. 构建语义车道集合和局部车道缓存
 * 3. 提供关键车提取、最近车道、参考线等查询能力
 * 4. 执行行为预测和开环轨迹预测
 * 5. 为 MPDM/EUDM/SSC 提供统一上游输入
 */
...
class SemanticMapManager {
 public:
  ...
  inline std::vector<int> fixed_navi_path() const {
 ...
 private:
  ...
  ErrorType GetLaneBySampledPoints(const vec_Vecf<2> &samples,
                                   const bool &is_high_quality,
                                   common::Lane *lane) const;
  std::vector<int> ResolveNavigationPath(
