...
def generate_launch_description():
    ...
    epsilon_share_dir = get_package_share_directory('epsilon_planning')
    ...
    guarded_cmd_vel_topic = LaunchConfiguration('guarded_cmd_vel_topic')
    ...
    scurm_nav_to_pose_bt = os.path.join(
        epsilon_share_dir,
        'behavior_tree',
        'ackermann_scurm_epsilon_recovery.xml',
    )
    scurm_nav_through_poses_bt = os.path.join(
        epsilon_share_dir,
        'behavior_tree',
        'ackermann_scurm_epsilon_through_poses.xml',
    )
    ...
    declare_params_file = DeclareLaunchArgument(
        ...
        default_value=os.path.join(
            epsilon_share_dir,
            'config',
            'nav2_gen0_scurm_epsilon_params.yaml',
        ),
        description='Full path to the EPSILON-aware Gen0 Nav2 parameter file.',
    )
    ...
    declare_nav2_cmd_vel_topic = DeclareLaunchArgument(
        'nav2_cmd_vel_topic',
        default_value='/control/nav2_cmd_vel_raw',
        description='Raw Nav2 velocity command published before EPSILON arbitration.',
    )
    declare_guarded_cmd_vel_topic = DeclareLaunchArgument(
        'guarded_cmd_vel_topic',
        default_value='/control/cmd_vel_raw',
        description='Command topic checked by nav2_pose_guard before reaching the robot.',
    )
    declare_final_cmd_vel_topic = DeclareLaunchArgument(
        'final_cmd_vel_topic',
    ...
    load_nodes = GroupAction(
        actions=[
            ...
            Node(
                ...
                arguments=['--ros-args', '--log-level', log_level],
                remappings=remappings
                + [('cmd_vel', 'cmd_vel_nav'), ('cmd_vel_smoothed', nav2_cmd_vel_topic)],
            ),
            Node(
                package='gen0_main',
