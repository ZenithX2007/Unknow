...
plan and executes only registered skills: `navigate`, `forward`, `rotate`,
`search_object`, and `stop`. The LLM never emits ROS interfaces or raw velocity
commands. `navigate` uses
...
`前进 5m` is a bounded, relative-distance motion. `左旋转 90度` and `右旋转 90度`
use a bounded driving arc because the Gen0 vehicle has Ackermann steering and
cannot safely rotate in place. The LLM never publishes velocity commands; a
trusted adapter owns the fixed speed, duration, cancellation, and zero-velocity stop.
